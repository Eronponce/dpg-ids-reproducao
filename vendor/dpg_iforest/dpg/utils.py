...
def change_edge_color(graph, source_id, target_id, new_color, new_width):
    ...
    for i, line in enumerate(graph.body):
        if f'{source_id} -> {target_id}' in line:
            # Remove existing attributes to start fresh or clean up
            # This regex removes everything inside [] including the brackets
            clean_line = re.sub(r'\s*\[.*?\]', '', line.rstrip())
            
            # Create the new line with all attributes
            # Note: We use space separation for attributes in DOT, not commas (though commas are allowed, mixing them can be tricky)
            # Using standard format: [attr="val" attr2="val2"]
            new_line = f'{clean_line} [color="{new_color}" penwidth="{new_width}" fontcolor="{new_color}" label=""]'
