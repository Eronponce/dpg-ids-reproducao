...
def test_base_sklearn(datasets, n_learners, decimal_threshold, contamination, seed, n_jobs=1, file_name=None, plot=False, save_plot_dir="examples/", attribute=None, communities=False, class_flag=False, predicates="feature_operator", mode="global_inliers", mode_graph="all", mode_score="log2", edge_attribute=None):
    """
    Trains an Isolation Forest model on a selected dataset, evaluates its performance, and optionally plots the DPG.

    Args:
    datasets: The path to the custom dataset to use.
    n_learners: The number of trees in the Isolation Forest.
    decimal_threshold: Decimal precision of each feature.
    contamination: Rate of outliers.
    seed: Random seed for reproducibility.
    n_jobs: Number of jobs to run in parallel for IsolationForest.
    file_name: The name of the file to save the evaluation results. If None, prints the results to the console.
    plot: Boolean indicating whether to plot the DPG.
    save_plot_dir: Directory to save the plot image.
    attribute: A specific node attribute to visualize.
    communities: Boolean indicating whether to visualize communities.
    class_flag: Boolean indicating whether to highlight class nodes.
    predicates: Type of predicate.
    mode: Analysis mode (e.g., global, local_outliers).
    mode_graph: Graph mode (all or last_decisions).
    mode_score: Score mode (e.g., log2, freq_pes).
    edge_attribute: A specific edge attribute to visualize.
    ...
    iForest = IsolationForest(n_estimators=n_learners, random_state=seed, contamination=contamination, n_jobs=n_jobs)  
    iForest.fit(data)
